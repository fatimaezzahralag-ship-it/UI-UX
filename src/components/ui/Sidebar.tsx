...
import {
  ...
  Layers, Clock, Video, ChevronLeft, ChevronRight,
  Menu
} from 'lucide-react';
import unismartLogo from '../../assets/unismart-logo.png';
      ...
      <div className="h-16 flex items-center justify-between px-4 border-b border-ink-800">
        {!collapsed && (
          <Link to="/" aria-label="Go to home" className="flex items-center">
            <img src={unismartLogo} alt="UniSmart" className="h-8 w-auto max-w-[150px] object-contain" />
          </Link>
        )}
        {collapsed && (
          <div className="w-8 h-8 rounded-lg bg-white p-1 overflow-hidden mx-auto">
            <img src={unismartLogo} alt="UniSmart" className="h-full w-full object-cover object-left" />
          </div>
        )}
        {!collapsed && (
